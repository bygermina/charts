...
export const drawAxes = ({
  ...
  try {
    ...
    ctx.lineTo(clippedWidth, chartHeight);
    ctx.moveTo(0, 0);
    ...
    const isTimestampValue = isTimestamp(min, max);

    const xTickValues = xAxisScale.ticks(xTicks);
    let xTickFormat: (value: number) => string;

    if (isTimestampValue) {
      const formatter = getTimeFormatter(min as number, max as number);
